...
from cohorts.signals.account import (
    signup_code_sent, 
    signup_code_used, 
    user_signed_up,
...
from cohorts.forms.account import (
    ChangePasswordForm,
    LoginUsernameForm,
    PasswordResetForm,
...
from cohorts.hooks import hookset
from cohorts.mixins import LoginRequiredMixin
from cohorts.models.accounts import (
    Account,
    AccountDeletion,
    EmailAddress,
    EmailConfirmation,
...
from cohorts.utils import default_redirect, get_form_data
...
class PasswordMixin(object):
    ...
    def after_change_password(self):
        user = self.get_user()
        password_changed.send(sender=self, user=user)
        if settings.COHORTS_NOTIFY_ON_PASSWORD_CHANGE:
    ...
    def create_password_history(self, form, user):
        if settings.COHORTS_PASSWORD_USE_HISTORY:
...
class SignupView(PasswordMixin, FormView):
    ...
    fallback_url_setting = "COHORTS_SIGNUP_REDIRECT_URL"
    ...
    def get(self, *args, **kwargs):
        if self.request.user.is_authenticated:
            return redirect(default_redirect(self.request, settings.COHORTS_LOGIN_REDIRECT_URL))
        if not self.is_open():
            return self.closed()
    ...
    def form_valid(self, form):
        ...
        if settings.COHORTS_EMAIL_CONFIRMATION_REQUIRED and not email_address.verified:
            self.created_user.is_active = False
            self.created_user.save()
        self.create_account(form)
        self.create_password_history(form, self.created_user)
        self.after_signup(form)
        if settings.COHORTS_EMAIL_CONFIRMATION_EMAIL and not email_address.verified:
            self.send_email_confirmation(email_address)
        if settings.COHORTS_EMAIL_CONFIRMATION_REQUIRED and not email_address.verified:
            return self.email_confirmation_required_response()
        else:
            show_message = [
                settings.COHORTS_EMAIL_CONFIRMATION_EMAIL,
    ...
    def is_open(self):
        ...
        return settings.COHORTS_OPEN_SIGNUP
...
class LoginView(FormView):
    ...
    def get_success_url(self, fallback_url=None, **kwargs):
        if fallback_url is None:
            fallback_url = settings.COHORTS_LOGIN_REDIRECT_URL
        kwargs.setdefault("redirect_field_name", self.get_redirect_field_name())
        return default_redirect(self.request, fallback_url, **kwargs)
    ...
    def login_user(self, form):
        ...
        expiry = settings.COHORTS_REMEMBER_ME_EXPIRY if form.cleaned_data.get("remember") else 0
...
class LogoutView(TemplateResponseMixin, View):
    ...
    def get_redirect_url(self, fallback_url=None, **kwargs):
        if fallback_url is None:
            fallback_url = settings.COHORTS_LOGOUT_REDIRECT_URL
        kwargs.setdefault("redirect_field_name", self.get_redirect_field_name())
        return default_redirect(self.request, fallback_url, **kwargs)
...
class ConfirmEmailView(TemplateResponseMixin, View):
    ...
    def post(self, *args, **kwargs):
        ...
        if confirmed:
            ...
            if settings.COHORTS_EMAIL_CONFIRMATION_AUTO_LOGIN:
    ...
    def get_redirect_url(self):
        if self.user.is_authenticated:
            if not settings.COHORTS_EMAIL_CONFIRMATION_AUTHENTICATED_REDIRECT_URL:
                return settings.COHORTS_LOGIN_REDIRECT_URL
            return settings.COHORTS_EMAIL_CONFIRMATION_AUTHENTICATED_REDIRECT_URL
        else:
            return settings.COHORTS_EMAIL_CONFIRMATION_ANONYMOUS_REDIRECT_URL
...
class ChangePasswordView(PasswordMixin, FormView):
    ...
    fallback_url_setting = "COHORTS_PASSWORD_CHANGE_REDIRECT_URL"
...
class PasswordResetTokenView(PasswordMixin, FormView):
    ...
    token_generator = default_token_generator
    form_password_field = "password"
    fallback_url_setting = "COHORTS_PASSWORD_RESET_REDIRECT_URL"
...
class SettingsView(LoginRequiredMixin, FormView):
    ...
    def update_email(self, form, confirm=None):
        ...
        if confirm is None:
            confirm = settings.COHORTS_EMAIL_CONFIRMATION_EMAIL
    ...
    def get_success_url(self, fallback_url=None, **kwargs):
        if fallback_url is None:
            fallback_url = settings.COHORTS_SETTINGS_REDIRECT_URL
        kwargs.setdefault("redirect_field_name", self.get_redirect_field_name())
        return default_redirect(self.request, fallback_url, **kwargs)
...
class DeleteView(LoginRequiredMixin, LogoutView):
    ...
    def post(self, *args, **kwargs):
        ...
        messages.add_message(
            ...
            self.messages["account_deleted"]["text"].format(**{
                "expunge_hours": settings.COHORTS_DELETION_EXPUNGE_HOURS,
            })
        )
        return redirect(self.get_redirect_url())
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update(kwargs)
        ctx["ACCOUNT_DELETION_EXPUNGE_HOURS"] = settings.COHORTS_DELETION_EXPUNGE_HOURS
        return ctx
