...
import pytz
#from .signals.account import signals
from cohorts.conf import settings
from cohorts.fields import TimeZoneField
from cohorts.hooks import hookset
from cohorts.languages import DEFAULT_LANGUAGE
from cohorts.managers import EmailAddressManager, EmailConfirmationManager
from cohorts.signals.account import signup_code_sent, signup_code_used
...
class Account(models.Model):
    ...
    language = models.CharField(
        _("language"),
        max_length=10,
        choices=settings.COHORTS_LANGUAGES,
...
def user_post_save(sender, **kwargs):
    ...
    disabled = getattr(user, "_disable_account_creation", not settings.COHORTS_CREATE_ON_SAVE)
...
class EmailAddress(models.Model):
    ...
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="account_email_address")
    email = models.EmailField(max_length=254, unique=settings.COHORTS_EMAIL_UNIQUE)
    ...
    class Meta:
        ...
        if not settings.COHORTS_EMAIL_UNIQUE:
...
class EmailConfirmation(models.Model):
    ...
    def key_expired(self):
        expiration_date = self.sent + datetime.timedelta(days=settings.COHORTS_EMAIL_CONFIRMATION_EXPIRE_DAYS)
    ...
    def send(self, **kwargs):
        ...
        activate_url = "{0}://{1}{2}".format(
            protocol,
            current_site.domain,
            reverse(settings.COHORTS_EMAIL_CONFIRMATION_URL, args=[self.key])
        )
        ctx = {
            "email_address": self.email_address,
...
class AccountDeletion(models.Model):
    ...
    def expunge(cls, hours_ago=None):
        if hours_ago is None:
            hours_ago = settings.COHORTS_DELETION_EXPUNGE_HOURS
        ...
        for account_deletion in cls.objects.filter(date_requested__lt=before, user__isnull=False):
            settings.COHORTS_DELETION_EXPUNGE_CALLBACK(account_deletion)
    ...
    def mark(cls, user):
        ...
        account_deletion.save()
        settings.COHORTS_DELETION_MARK_CALLBACK(account_deletion)
